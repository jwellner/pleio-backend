...
class DownloadFileTestCase(PleioTenantTestCase):

    def setUp(self):
        super(DownloadFileTestCase, self).setUp()
    ...
    def test_download_file_anonymous(self):
        response = self.client.get("/file/download/{}".format(self.file.id))
        self.assertEqual(response.status_code, HTTPStatus.UNAUTHORIZED)
        self.assertFalse(hasattr(response, 'streaming_content'))

    def test_download_file_not_in_group(self):
