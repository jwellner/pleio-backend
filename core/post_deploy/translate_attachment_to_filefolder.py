...
def migrate_attachment(attachment_id):
    ...
    try:
        ...
        if attachment.attached:
            ...
            container = _maybe_entity_object(attachment.attached)
        ...
        print(format_exc())
        if attachment:
            logger.error("migrate_attachment error: %s %s@%s %s %s", tenant_schema(), attachment.name, attachment.attached, e.__class__, str(e))
        else:
            logger.error("migrate_attachment error: %s %s@%s %s %s", tenant_schema(), attachment_id, '???', e.__class__, str(e))


def _maybe_entity_object(entity):
    if not isinstance(entity, (Entity,)):
        return entity
    return Entity.objects.filter(pk=entity.pk).select_subclasses().first()
...
def schedule_calculate_checksum():
