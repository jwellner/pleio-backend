...
def schedule_user_request_delete_for_admin_mail(user, admin):
    from core.models import MailInstance
    MailInstance.objects.submit(UserRequestDeleteToAdminMailer,
                                mailer_kwargs={
                                    "user": user.guid,
                                    "admin": admin.guid
                                })
...
class UserRequestDeleteToAdminMailer(TemplateMailerBase):
    ...
    def get_template(self):
        return 'email/toggle_request_delete_user_requested_admin.html'
