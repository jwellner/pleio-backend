...
def schedule_assign_admin_for_admin_mail(user, admin, sender):
    from core.models import MailInstance
    MailInstance.objects.submit(UserAssignAdminForAdminMailer,
                                mailer_kwargs={
                                    "user": user.guid,
                                    "admin": admin.guid,
                                    "sender": sender.guid,
...
class UserAssignAdminForAdminMailer(TemplateMailerBase):
    ...
    def get_template(self):
        return "email/user_role_admin_assigned_for_admins.html"
    ...
    def get_subject(self):
        return gettext("A new site administrator was assigned for %(site_name)s") % {'site_name': config.NAME}
