...
def schedule_assign_admin_for_user_mail(user, sender):
    from core.models import MailInstance
    MailInstance.objects.submit(UserAssignAdminForSelfMailer,
                                mailer_kwargs={
                                    "user": user.guid,
                                    "sender": sender.guid
                                })
...
class UserAssignAdminForSelfMailer(TemplateMailerBase):
    ...
    def get_template(self):
        return "email/user_role_admin_assigned_for_user.html"
    ...
    def get_subject(self):
        return gettext("You're granted site administrator right on %(site_name)s") % {'site_name': config.NAME}
